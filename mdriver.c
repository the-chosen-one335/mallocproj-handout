/*
 * mdriver.c - CS3103 Malloc Project Driver
 *
...
int main(int argc, char **argv)
{
    ...
    int group_check = 1;  /* If set, check group structure (reset by -a) */
	...
	case 't': /* Directory where the traces are located */
	    ...
	    break;
        case 'a': /* Don't check group structure */
            group_check = 0;
            break;
    ...
    /*
     * Check and print group info
     */
    if (group_check) {
	/* Students must fill in their group information */
	if (!strcmp(group.groupno, "")) {
	    printf("ERROR: Please provide the information about your group in mm.c.\n");
	    exit(1);
	} else
	    printf("Group No: %s\n", group.groupno);
	if ((*group.name1 == '\0') || (*group.email1 == '\0')) {
	    printf("ERROR.  You must fill in all group member 1 fields!\n");
	    exit(1);
	}
	else
	    printf("Member 1: %s <%s>\n", group.name1, group.email1);

	if (((*group.name2 != '\0') && (*group.email2 == '\0')) ||
	    ((*group.name2 == '\0') && (*group.email2 != '\0'))) {
	    printf("ERROR.  You must fill in all or none of the group member 2 fields!\n");
	    exit(1);
	}
	else if (*group.name2 != '\0')
	    printf("Member 2: %s <%s>\n", group.name2, group.email2);

	if (((*group.name3 != '\0') && (*group.email3 == '\0')) ||
	    ((*group.name3 == '\0') && (*group.email3 != '\0'))) {
	    printf("ERROR.  You must fill in all or none of the group member 3 fields!\n");
	    exit(1);
	}
	else if (*group.name3 != '\0')
	    printf("Member 3: %s <%s>\n", group.name3, group.email3);
    }
...
static trace_t *read_trace(char *tracedir, char *filename)
{
    ...
    ignore += fscanf(tracefile, "%d", &(trace->sugg_heapsize)); /* not used */
    ignore += fscanf(tracefile, "%d", &(trace->num_ids));
    ignore += fscanf(tracefile, "%d", &(trace->num_ops));
    ignore += fscanf(tracefile, "%d", &(trace->weight));        /* not used */
...
static void usage(void)
{
    ...
    fprintf(stderr, "\t-a         Don't check the group structure.\n");
